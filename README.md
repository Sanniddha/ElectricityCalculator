...
$$\text{Energy generated (in kW)} = \text{Area of each solar panel (in $m^2$)} * \text{No.of solar panels} * \text{Efficiency of the solar panel(%)} * \text{Monthly avg.solar radiation (in kW/$m^2$)} * \text{Performance ratio}$$
